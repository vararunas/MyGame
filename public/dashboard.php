...
$error=null;$notice=null;$db=Connection::make();$companyId=(int)$_SESSION['company_id'];$_SESSION['dashboard_csrf']??=bin2hex(random_bytes(24));$csrf=$_SESSION['dashboard_csrf'];$engine=new GameEconomyEngine($db);$engine->sync();(new AiCompanyEngine($db))->seed();$engine->refreshObligations($companyId);
try{
