...
try{
 ...
 $sql="SELECT c.id,c.name,m.period,ROUND(COALESCE(m.revenue,0),2) cycle_revenue,
 ROUND(COALESCE((SELECT SUM(s.revenue) FROM sales_ledger s WHERE s.company_id=c.id AND s.period=m.period),0),2) ledger_revenue,
 ROUND(COALESCE((SELECT SUM(t.amount) FROM bank_account_transactions t JOIN company_bank_accounts a ON a.id=t.account_id WHERE a.company_id=c.id AND t.transaction_type='sales_revenue' AND t.description LIKE CONCAT('%',m.period,'%')),0),2) bank_revenue
 FROM companies c JOIN company_monthly_cycles m ON m.company_id=c.id ORDER BY m.period DESC,c.name LIMIT 100";
 $rows=$db->query($sql)->fetchAll();
 foreach($rows as &$r){$lr=(float)$r['ledger_revenue'];$cr=(float)$r['cycle_revenue'];$br=(float)$r['bank_revenue'];$r['level']=(abs($lr-$cr)>.01||abs($lr-$br)>.01)?'error':'ok';}unset($r);
