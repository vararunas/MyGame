...
try{
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $timeAction=(string)($_POST['time_action']??'');if($timeAction!==''){$days=['day'=>1,'week'=>7,'month'=>31][$timeAction]??0;if($timeAction==='reset'){$engine->resetTestOffset();$timeNotice='Testinis laikas grąžintas į realią datą.';}elseif($days>0){$engine->advance($days);$timeNotice='Testinis laikas pasuktas pirmyn.';}}else{$effectiveFrom=(string)($_POST['effective_from']??date('Y-m-d'));$repo->updateSection($section,$_POST['values']??[],$effectiveFrom);$saved=true;}
 }
