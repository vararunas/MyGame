...
use MyGame\Infrastructure\Database\Connection;use MyGame\Infrastructure\State\StateTreasuryRepository;
$error=null;$summary=['institutions'=>[],'total'=>0,'income'=>0,'month'=>0];$transactions=[];$breakdown=[];
try{$repo=new StateTreasuryRepository(Connection::make());$summary=$repo->summary();$transactions=$repo->transactions(100);$breakdown=$repo->breakdown();}catch(Throwable$e){$error=$e->getMessage();}
